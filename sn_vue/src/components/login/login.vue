        ...
        <div class="forms">
          ...
          <div class="pc-login">
            <div class="uname-login">
              ...
              <div class="uname">
                <i></i>
                <input type="text" name="uname" placeholder="用户名" v-model="user.uname">
                <em></em>
              </div>
              <div v-if="unameWarn.length" class="unameWarn" v-text="unameWarn"></div>
              <div class="upwd">
                <i></i>
                <input type="password" name="upwd" placeholder="密码" v-model="user.upwd">
                <em></em>
              </div>
...
export default {
  ...
  methods: {
    login() {
      ...
      if (!(this.user.upwd.length >= 6 && this.user.upwd.length <= 13)) {
        this.upwdWarn = "请输入6~13位密码";
        return;
      } else this.upwdWarn = "";
      this.$axios
        .post("/user/login", this.$qs.stringify(this.user), {   //使用nodejs 需要将对象 编码
          headers: {
        ...
        .then(res => {
          ...
          if (res.data.code != 200) {
            ...
            if(this.$route.params.from =='/register') this.$router.push('/index')
            else if(this.$route.params.from) this.$router.push(this.$route.params.from)
